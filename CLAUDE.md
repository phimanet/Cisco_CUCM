...
- [ ] [P1][Planned] v1.01 VeraSMART (Calero 2026 R1) LAB-only automation: expand the existing scaffold into a template-person workflow backed by a current VeraSMART Personnel export. A manager selects a target employee and a known-good template employee; preview copies the template's exact Cost Center while preserving the target's email, employee number, name, LDAP-managed Login ID, and other identity fields. Generate Personnel and EZ-Burst files for manual validation first. The existing six-hour VeraSMART LDAP import owns Email and Login ID, so neither is mapped in the Personnel correction import. Personnel correction uses Windows domain account (`AHS\username`) as the match, existing-records-only, no add, and no auto-deletion; mapped updates are Windows domain account, Cost Center, and EZ-Burst option. Export email is used only for the separate EZ-Burst distribution import. `Login disabled` is a leave-of-absence exception that LDAP resync does not repair; every approved correction must force the target to `Login disabled = No` rather than copy the template value. Add it to the generated import only after the column-mapping wizard confirms it is importable and establishes whether the accepted value is `No`, `0`, or `false`; otherwise the workflow must report a required manual VeraSMART action and cannot claim full completion. After both manual imports pass, add persistent queued SFTP staging, atomic delivery, ordered Personnel-then-EZ-Burst scheduling, status/rejection tracking, Cost Center allow-list validation, export freshness/duplicate checks, preview/approval, and audit history. Confirmed existing delivery target: restricted SFTP account `cdruser` at `las-vrecas-p01.ahs.int:/`, mapping to Windows `D:\Verasmart Data\FTPRoot`; the account is already locked to that folder, so no broader filesystem access is required. Personnel exports and other PII remain outside Git with restricted retention. No production rollout until LAB validation is complete.
- VeraSMART validation rule (2026-09-24): after each LAB import, use VeraSMART `Help > View Status Log`, open the relevant `View Import Changes Log`, and verify the Personnel `Edited` tab. Confirm the target's before/after Cost Center change and ensure no unintended Email, Network Login, employee-number, name, location, or office-number edits occurred. Review `Added` and `Deleted` only when the import configuration permits those operations; the planned correction import remains existing-records-only with no auto-deletion. Retain the import-log result as the authoritative manual-validation evidence before enabling SFTP.
- VeraSMART builder workflow (2026-09-24): upload one current Personnel export, choose one known-good template employee, search/filter repeatedly, add or remove target employees from the visible queue, and generate only after the queue is complete. The generated outputs are exactly two manual files: Personnel Cost Center update and EZ-Burst Distribution update. The builder preserves each target's Windows domain account and uses the template's exact Cost Center; no SAML, Security Group, Login Disabled, or SFTP action is performed by this workflow.
- VeraSMART SFTP delivery folder structure confirmed (2026-09-24): operator created two subfolders under the delivery target — `D:\Verasmart Data\FTPRoot\personnel` and `D:\Verasmart Data\FTPRoot\ezburst` — separating Personnel Import files from EZ-Burst Distribution files. Future SFTP automation should deliver each generated file type to its matching subfolder.
- VeraSMART both import jobs (Personnel Import_CiscoAdmin, EZ-Burst Distribution) are now configured as scheduled/recurring imports in VeraSMART itself (2026-09-24), watching their respective `personnel`/`ezburst` subfolders. This means our SFTP automation only needs to atomically deliver the generated files into the correct subfolder in the right order (Personnel first, then EZ-Burst) — VeraSMART's own schedule will pick them up and run the import; our code does not need to trigger the import itself.
- Confirmed VeraSMART recurrence (2026-09-24): **Personnel Import_CiscoAdmin runs every hour** (any time of day); **EZ-Burst Distribution runs once daily, only at 1:00 AM**. Approved delivery-timing design: the Personnel file can be delivered anytime (absorbed by the hourly cadence), but the EZ-Burst file must not be delivered any later than **11:30 PM** so at least one Personnel hourly run completes (applying the Cost Center correction) before the 1:00 AM EZ-Burst run groups users by Cost Center. If a Personnel/EZ-Burst pair is generated after 11:30 PM, the EZ-Burst file must be held and delivered for the *next* day's 1:00 AM run instead of the same night's. SFTP delivery implementation itself is paused/deferred; this scheduling design is confirmed and ready for when SFTP work resumes.
...
- **EZBurstOption raw import value confirmed**: the GUI dropdown label `E-mail links to EZ-Burst reports` is NOT the value the text importer accepts — importing that exact string was rejected as invalid, even though manually selecting it in the GUI and saving worked fine. The importer's accepted raw code is `Linked`. Live Personnel Import test on `Alfredo.Salcedo@amnhealthcare.com` succeeded with `EZBurstOption=Linked`, confirmed via the Personnel Details view showing Cost Center `Allied SAM`, Windows domain account `AHS\Alfredo.Salcedo`, and EZ-Burst option `E-mail links to EZ-Burst reports` (the GUI-rendered label for the `Linked` code). Both CSV generators (CUCM live-search flow and Personnel-export template flow) now emit `Linked` for this field — this is the confirmed working value going forward.
- **EZ-Burst Distribution import validated end-to-end**: dropped the unused `Department` column (always blank, mapped to `[Ignored]` in the wizard); file is now 3 columns (`DistributionListName`, `EmailAddress`, `CostCenter`) mapped to `Distribution list name`, `E-mail address`, `Cost Center`.
- **Both LAB imports (Personnel Import and EZ-Burst Distribution) confirmed successful** on the same test record (`Alfredo.Salcedo@amnhealthcare.com`), completing full manual validation of the CUCM-driven VeraSMART builder workflow end-to-end. This is now the confirmed working baseline for both generated file formats.
- **SFTP auto-delivery implemented and live-validated (2026-09-24)**: on clicking Generate Queued CSV Files, the portal now delivers both CSVs directly via SFTP (`paramiko`) to `las-vrecas-p01.ahs.int:22`, landing in `/personnel` and `/ezburst` (mapped to `D:\Verasmart Data\FTPRoot\personnel`/`\ezburst`). Confirmed via a live test — operator saw the delivered file on the SFTP server in the correct folders. Credentials (`VERASMART_SFTP_HOST/PORT/USER/PASSWORD/PERSONNEL_DIR/EZBURST_DIR`) are in `.env` on both LAB and PROD; `paramiko` installed in both venvs; both servers pulled and restarted. EZ-Burst delivery still respects the 11:30 PM Pacific cutoff (holds in `data/verasmart_ezburst_pending.json`, gitignored, delivered automatically by a background thread once past the cutoff window).
- Next watch item: confirm VeraSMART's own scheduled import jobs (Personnel hourly, EZ-Burst 1:00 AM daily) actually pick up and successfully process the SFTP-delivered files without manual "Save and Run Now" intervention — this is the last unverified link now that delivery itself works.
...
- Results include only missing email, malformed email, and authoritative LDAP-not-found users, with name, Genesys ID, email, state, division, AD status, and candidate reason; username remains internal for pre-update identity validation.
...
- Before inactivation, the worker snapshots WebRTC phones whose authoritative `webRtcUser.id` matches the exact user. After the user is confirmed Inactive, each phone is re-read and deleted if the association still matches or Genesys cleared it during inactivation; reassigned, renamed, or non-WebRTC phones are blocked. User/phone outcomes are tracked and audited separately.
