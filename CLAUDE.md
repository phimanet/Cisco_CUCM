...
- [ ] [P1][Planned] v1.01 VeraSMART (Calero 2026 R1) LAB-only automation: expand the existing scaffold into a template-person workflow backed by a current VeraSMART Personnel export. A manager selects a target employee and a known-good template employee; preview copies the template's exact Cost Center while preserving the target's email, employee number, name, LDAP-managed Login ID, and other identity fields. Generate Personnel and EZ-Burst files for manual validation first. Personnel import uses email match, existing-records-only, no add, and no auto-deletion; mapped updates are Cost Center, EZ-Burst option, and target Windows domain account (`AHS\username`). After both manual imports pass, add persistent queued SFTP staging, atomic delivery, ordered Personnel-then-EZ-Burst scheduling, status/rejection tracking, Cost Center allow-list validation, export freshness/duplicate checks, preview/approval, and audit history. Confirmed existing delivery target: SFTP `las-vrecas-p01.ahs.int:/` maps to Windows `D:\Verasmart Data\FTPRoot` and is already in use. Personnel exports and other PII remain outside Git with restricted retention. No production rollout until LAB validation is complete.
